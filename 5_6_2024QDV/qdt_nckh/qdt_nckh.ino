...
float kp_eth = 0.1, ki_eth = 0.001, kd_eth = -0.001;
...
float xd_dot, yd_dot, xd = 1, yd = -1,thetad=0,theta0,x0d_dot,y0d_dot;
float omega = 0.06,R=1.25, r = 0.075, heso=0.01;
...
void quydao(float &xd_dot, float &yd_dot, float &xd, float &yd) {
  ...
  xd=R*cos(omega*t);
  yd=R*sin(omega*t);
  thetad=1.57;//atan2(-xd,-yd);//rad(qd tron)
  if (thetad< 0) {   thetad +=2*3.14;  }// giu cho goc o tu 0-2pi

  // x0d_dot=-R*omega*sin(omega);
  // y0d_dot=R*omega*cos(omega);
