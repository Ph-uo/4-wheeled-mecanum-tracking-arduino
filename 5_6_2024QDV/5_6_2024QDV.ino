...
float kp_ex = 0.03, ki_ex = 0.001, kd_ex = -0.0001;
float kp_ey = 0.03, ki_ey = 0.001, kd_ey = -0.0001;
float error_ex, error_prevex = 0, integ_ex, integ_prevex = 0;
float error_ey, error_prevey = 0, integ_ey, integ_prevey = 0;
float ex1 = 0, ey1 = 0;

float kp1=0.45, ki1=0.2, kd1=0;
...
float xd_dot, yd_dot, xd = 1, yd = -1;
float heso = 0.15, r = 0.075;
float theta = 0, thetad_dot = 0;
float vx = 0, vy = 0, w = 0, xe = 0, ye = 0, ex = 0, ey = 0;
//float x = 0, y = 0;
float x = 1, y = -1;
...
void quydao_HV(float &xd_dot, float &yd_dot, float &xd, float &yd) {
  ...
  vx1 = (cos(theta) * vxa) + (sin(theta)) * vya;
  vy1 = (-sin(theta) * vxa) + (cos(theta)) * vya;
...
void Data_camera_esp() {
  if (Serial3.available() > 0) {
    ...
    // Tách giá trị x và yn9 từ chuỗi nhận được
